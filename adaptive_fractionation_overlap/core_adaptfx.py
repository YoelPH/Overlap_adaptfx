...
def adaptive_fractionation_core(fraction: int, volumes: np.ndarray, accumulated_dose: float, number_of_fractions: int = 5, min_dose: float = 7.5, max_dose: float = 9.5, mean_dose:float  = 8, dose_steps: float = 0.25, alpha: float = 1.072846744379587, beta:float = 0.7788684130749829):
    ...
    Args:
        ...
        dose_steps (float, optional): difference between the deliverable doses. Defaults to 0.25.
        alpha (float, optional): alpha value of gamma distribution. Defaults to 1.072846744379587.
        beta (float, optional): beta value of gamma distribution. Defaults to 0.7788684130749829.
    ...
    if goal - accumulated_dose < (number_of_fractions + 1 - fraction) * min_dose:
        ...
        for state, fraction_state in enumerate(np.arange(number_of_fractions, fraction-1, -1)):
            if (state == number_of_fractions - 1):  # first fraction with no prior dose delivered so we dont loop through dose_space
                overlap_penalty = penalty_calc_matrix(delivered_doses, volume_space, min_dose, mean_dose) #This means only values over min_dose get a penalty. Values below min_dose do not get a reward
                actual_penalty = penalty_calc_single_volume(delivered_doses, min_dose, mean_dose, actual_volume)
                ...
                for indexer, poss_accum_doses in enumerate(dose_space_adapted):  # loop through all possible accumulated doses
                    ...
                    overlap_penalty = penalty_calc_matrix(delivered_doses_clipped, volume_space, min_dose, mean_dose) #This means only values over min_dose get a penalty.
                    actual_penalty = penalty_calc_single_volume(delivered_doses_clipped, min_dose, mean_dose, actual_volume)
                    ...
                    values_actual_frac_short = -overlap_penalty + future_values + penalties
                ...
                for tumor_index, tumor_value in enumerate(dose_space):  # this and the next for loop allow us to loop through all states
                    ...
                    if state != 0:
                        ...
                        last_penalty = penalty_calc_single(best_action, min_dose, mean_dose, volume_space)
                        underdose_penalty = 0
                        overdose_penalty = 0
    ...
    penalty_added = penalty_calc_single(physical_dose, min_dose, mean_dose, actual_volume)
...
def adaptfx_full(volumes: list, number_of_fractions: int = 5, min_dose: float = 7.5, max_dose: float = 9.5, mean_dose: float = 8, dose_steps: float = 0.25, alpha: float = 1.072846744379587, beta:float = 0.7788684130749829):
    ...
    for index, frac in enumerate(range(1,number_of_fractions +1)):
        if frac != number_of_fractions:
            ...
            [policies, policies_overlap, volume_space, physical_dose, penalty_added, values, dose_space, probabilities, final_penalty, values_actual_frac, dose_space_adapted]  = adaptive_fractionation_core(fraction = frac, volumes = volumes,accumulated_dose = accumulated_doses[index], number_of_fractions= number_of_fractions, min_dose = min_dose, max_dose = max_dose, mean_dose = mean_dose, dose_steps = dose_steps, alpha = alpha, beta = beta)
        physical_doses[index] = physical_dose
    total_penalty = 0
    for index, dose in enumerate(physical_doses):
        total_penalty -= penalty_calc_single(dose, min_dose, mean_dose, volumes[-number_of_fractions+index])
    return physical_doses, accumulated_doses, total_penalty


def precompute_plan(fraction: int, volumes: np.ndarray, accumulated_dose: float, number_of_fractions: int = 5, min_dose: float = 7.5, max_dose: float = 9.5, mean_dose:float  = 8, dose_steps = 0.25, alpha: float = 1.072846744379587, beta:float = 0.7788684130749829):
