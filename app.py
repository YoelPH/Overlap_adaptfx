...
with left:
    ...
    steepness_penalty = st.text_input('steepness_penalty of the DVH dose gradient', '-0.5', help = 'insert the steepness_penalty of DVH gradient')
    steepness_benefit = st.text_input('steepness_benefit of the DVH dose gradient', '-0.1', help = 'insert the steepness_benefit of DVH gradient')
...
if st.button('compute optimal dose', help = 'takes the given inputs from above to compute the optimal dose'):
    ...
    if function == 'actual fraction calculation':
        ...
        [physical_doses, accumulated_doses, total_penalty] = af.adaptfx_full(volumes = np.array(overlaps), number_of_fractions = int(fractions), steepness_penalty = float(steepness_penalty), steepness_benefit = float(steepness_benefit), min_dose = float(minimum_dose), max_dose = float(maximum_dose), mean_dose = float(mean_dose), dose_steps = float(dose_steps), minimum_benefit = float(minimum_benefit))
        ...
        st.metric(label = "penalty", value = str(total_penalty) + 'ccGy', delta = np.round(total_penalty + (np.array(overlaps[-int(fractions):])*(float(mean_dose)- float(minimum_dose))).sum(),2))
